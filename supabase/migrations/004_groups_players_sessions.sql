-- Phase 4: Groups, Players, and Game Sessions
-- Migration 004
-- Date: 2026-04-22
...
CREATE INDEX idx_groups_sport ON groups(sport);
...
CREATE INDEX idx_players_rating ON players(default_rating);
...
CREATE INDEX idx_sessions_group ON game_sessions(group_id);
CREATE INDEX idx_sessions_date ON game_sessions(session_date DESC);
...
-- Add player_id FK to link ratings to structured players
...
-- Keep existing player_name/sport for backward compatibility
-- Will be nullable for new player-based ratings
...
-- Add new unique constraint for player-based ratings
-- Note: This allows both old (name+sport) and new (player_id) approaches temporarily
...
-- GROUPS: All authenticated users can view
...
-- GAME SESSIONS: All authenticated users can view and create
...
-- SESSION PLAYERS: All authenticated users can manage roster
...
-- Function to get player's final rating
-- Returns calculated rating from graders OR default_rating if not graded
...
-- Function to get session roster with ratings
...
-- View: Groups with player counts
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE NOTICE 'SUCCESS: player_ratings.player_id column added';
  END IF;

  -- Check functions exist
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_player_final_rating') THEN
    RAISE NOTICE 'SUCCESS: get_player_final_rating function created';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_session_roster') THEN
    RAISE NOTICE 'SUCCESS: get_session_roster function created';
  END IF;
END $$;
